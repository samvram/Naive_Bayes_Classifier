...
def start_counting(file):
    ...
    speakers = []
    bag_of_words = []
    unnecessary_words = ['and', 'is', 'are', 'in', 'the', '', '.', '?', ',', '!', ';', ':', 'this', 'here', 'there', 'where',
                         'why', 'who', 'what', 'when', 'that']


    for speech in speech_list:
        name = speech.split(' ')[0]
        if name not in speakers and name not in unnecessary_words:
    ...
    for speech in speech_list:
        ...
        for word in words:
            if word not in bag_of_words and word not in unnecessary_words:
